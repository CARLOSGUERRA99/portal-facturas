      ...
      if (m) {
        ...
        if (buf && buf.length > 200) {
          ...
          if (preview.includes('<?') || preview.includes('<cfdi') || preview.includes('<Comprobante')) {
            xmlUrl = await subirArchivoR2(buf, `facturas/gasmaz_${ticketId}.xml`, 'application/xml');
          }
        }
      }
    }
    ...
    if (pdfLink) {
      ...
      if (m) {
        ...
        if (directBuf && directBuf.toString('latin1', 0, 4) === '%PDF') {
          pdfUrl = await subirArchivoR2(directBuf, `facturas/gasmaz_${ticketId}.pdf`, 'application/pdf');
        } else {
          ...
          if (pdfBuffer) {
            pdfUrl = await subirArchivoR2(pdfBuffer, `facturas/gasmaz_${ticketId}.pdf`, 'application/pdf');
          }
