...
async function dump(page, label) {
  ...
  if (buf) {
    const u = await subirArchivoR2(buf, `debug/horizon_probe_${label}_${Date.now()}.png`, 'image/png');
    console.log(`📸 [${label}]: ${u}`);
  }
...
(async () => {
  ...
  console.log('🌐 Cargando grupohorizon.com.mx ...');
  const r = await page.goto('https://www.grupohorizon.com.mx', { waitUntil: 'load', timeout: 30000 });
  console.log('Status:', r.status());
  await page.waitForTimeout(2000);
  await dump(page, 'p1_home');

  console.log('\n➡️ Navegando a Facturación...');
  await page.goto('https://www.grupohorizon.com.mx/facturaci%C3%B3n', { waitUntil: 'load', timeout: 30000 });
  await page.waitForTimeout(2000);
  await dump(page, 'p2_facturacion');
