...
(async () => {
  ...
  console.log('➡️ Click "Facturar sin registro"...');
  await page.evaluate(() => {
  ...
  const inputs = await page.evaluate(() => {
    return Array.from(document.querySelectorAll('input,select,textarea,button')).map(el => ({
      tag: el.tagName, id: el.id, name: el.name, type: el.type, placeholder: el.placeholder || el.getAttribute('aria-label'), texto: (el.textContent||'').trim().slice(0,40),
  ...
  console.log('📸', await subirArchivoR2(buf, `debug/enerfueltech_sinregistro_${Date.now()}.png`, 'image/png'));
