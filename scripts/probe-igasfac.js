...
async function dump(page, label) {
  ...
  if (buf) {
    const u = await subirArchivoR2(buf, `debug/igasfac_probe_${label}_${Date.now()}.png`, 'image/png');
    console.log(`📸 [${label}]: ${u}`);
  }
...
(async () => {
  ...
  console.log('🌐 Cargando igasfac.com.mx ...');
  const r = await page.goto('https://www.igasfac.com.mx', { waitUntil: 'load', timeout: 30000 });
  console.log('Status:', r.status());
  await page.waitForTimeout(2000);
  await dump(page, 'p1_home');

  console.log('\n➡️ Revisando flujo de "Registrarse"...');
  await page.goto('https://www.igasfac.com.mx/Registro/ValidacionTicket', { waitUntil: 'load', timeout: 30000 });
  await page.waitForTimeout(1500);
  await dump(page, 'p2_registro');
