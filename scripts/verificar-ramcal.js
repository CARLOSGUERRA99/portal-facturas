...
imap.once('ready', () => {
  imap.openBox('INBOX', false, (err) => {
    ...
    imap.search([['HEADER', 'FROM', 'e07932.facturas']], (err2, uids) => {
      if (err2 || !uids.length) { console.error('❌ No se encontró correo de RAMCAL'); process.exit(1); }
      ...
      fetch.on('message', (msg) => {
        msg.on('body', (stream) => {
          simpleParser(stream, async (perr, parsed) => {
            ...
            const cpReceptor = (xml.match(/<(?:cfdi:)?Receptor[^>]*\sDomicilioFiscalReceptor="([^"]+)"/i) || [])[1];
            ...
            console.log('CP Receptor (DomicilioFiscalReceptor):', cpReceptor);
            ...
            const xmlUrl = await subirArchivoR2(xmlBuf, `facturas/ramcal_${uuid}.xml`, 'application/xml');
            const pdfUrl = pdfBuf ? await subirArchivoR2(pdfBuf, `facturas/ramcal_${uuid}.pdf`, 'application/pdf') : null;
            console.log('\n☁️ XML:', xmlUrl);
