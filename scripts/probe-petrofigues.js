...
async function dump(page, label) {
  ...
  if (buf) {
    const u = await subirArchivoR2(buf, `debug/petrofigues_probe_${label}_${Date.now()}.png`, 'image/png');
    console.log(`📸 [${label}]: ${u}`);
  }
...
(async () => {
  ...
  console.log('🌐 Cargando petrofigues.com/facturacion.html ...');
  const r = await page.goto('https://petrofigues.com/facturacion.html', { waitUntil: 'load', timeout: 30000 });
