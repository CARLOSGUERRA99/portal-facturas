...
async function procesarTicketsPorCorreo() {
  ...
  try {
    ...
    for (const t of atascados) {
      // Sin error_msg esto queda en 'error' sin ninguna pista de qué pasó — la
      // factura SÍ se generó en el portal, solo el correo con el CFDI no llegó
      // (o no coincidió con los patrones de esCFDI en mail/imap.js). Confundía
      // a simple vista con un fallo real del bot (visto en vivo: tickets #327/#330).
      // ⚠️ Si el bot alcanzó a registrar "Factura generada", el CFDI YA EXISTE y
      // reintentar emite un duplicado. El mensaje genérico decía "o reintenta",
      // que en ese caso es justo lo que no hay que hacer. Se distingue mirando
      // el último intento (pasó con el #315 y el #327, ambos ya timbrados).
      const [[ultimo]] = await db.query(
        "SELECT resultado, mensaje FROM ticket_intentos WHERE ticket_id = ? ORDER BY id DESC LIMIT 1",
        [t.id]
      );
      const yaTimbrado = ultimo && /factura generada|timbrad/i.test(String(ultimo.mensaje || ""));
      const aviso = yaTimbrado
        ? `El CFDI de ${t.comercio || "este comercio"} SÍ se generó en el portal (el bot lo confirmó) pero el correo no llegó en 60 minutos. NO reintentar: se emitiría un duplicado. Buscarlo en el portal o correr scripts/reconciliar-correo.js cuando llegue.`
        : `El correo con el CFDI de ${t.comercio || "este comercio"} no llegó en 60 minutos. El portal probablemente sí generó la factura — revisa el buzón de correo o reintenta.`;
      await db.query(
        "UPDATE tickets SET status = 'error', reintento_programado = NULL, error_msg = ? WHERE id = ?",
        [aviso, t.id]
      );
      await crearNotificacion(
        ...
        "factura_error",
        yaTimbrado
          ? `Tu factura de ${t.comercio || "comercio"} ya se generó, pero el correo con el archivo no ha llegado. La estamos recuperando — no hace falta que hagas nada.`
    ...
    [rows] = await db.query(
      // El RFC sale del CLIENTE si lo hay, y solo si no, del login: es el mismo
      // COALESCE que usa lib/facturacion.js. Hace falta para comprobar que el
      // CFDI que llegó por correo esté a nombre de quien toca.
  ...
  for (const ticket of rows) {
    ...
    try {
      ...
      // El CFDI que llega por CORREO se comprueba igual que el que baja el bot.
      // Antes no: lib/facturacion.js verificaba receptor e importe, pero esta
      // rama insertaba solo las dos URLs, y por aquí entran AutoZone, Orler,
      // Carl's Jr y Home Depot. Los tickets #215 y #228 quedaron con uuid,
      // total y emisor a null — sin dato alguno de a nombre de quién se emitió.
      const cfdi = xmlBuffer ? leerCFDI(xmlBuffer) : null;
      const problemas = cfdi ? verificarCFDI(cfdi, { rfcEsperado: ticket.rfc, totalEsperado: Number(datos.total) || 0 }) : [];
      ...
      if (problemas.length) console.log(`${graves.length ? '🛑' : '⚠️ '} CFDI por correo #${ticket.id}: ${resumen}`);

      const datosCfdi = [
        cfdi?.uuid || null, cfdi?.receptorRfc || null, cfdi?.emisorRfc || null,
      ...
      try {
        await db.query(
          `INSERT INTO facturas (user_id, ticket_id, comercio, pdf_url, xml_url, status,
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          // ⚠️ MISMO slice(0,50) que lib/facturacion.js, y por la MISMA razón:
          // `facturas.comercio` es varchar(50) y hay comercios más largos. Aquí
          // faltaba, y el fallo es peor que allí: para cuando se ejecuta este
          // INSERT el correo YA se marcó como leído, así que el ER_DATA_TOO_LONG
          // sube al catch de abajo, se reporta como "no llegó correo" y el CFDI
          // queda inalcanzable para siempre (la búsqueda IMAP es solo UNSEEN).
          // Caso real: ticket #380, "ESTACION DE SERVICIO PUERTA GRANDE, S. DE
          // R.L. DE C.V." (54 caracteres) — timbrado, en el buzón, y perdido.
          [ticket.user_id, ticket.id, String(ticket.comercio || '').slice(0, 50), pdfUrl, xmlUrl, "completado", ...datosCfdi]
        );
        if (graves.length) {
          await crearNotificacion(ticket.user_id, 'factura_error',
        ...
        if (dupErr.code === "ER_DUP_ENTRY") {
          ...
          if (existente && !existente.xml_url && xmlUrl) {
            await db.query(
              `UPDATE facturas SET pdf_url=?, xml_url=?, status='completado',
                                   uuid=?, receptor_rfc=?, emisor_rfc=?, emisor_nombre=?, total=?, serie_folio=?, fecha_timbrado=?, verificacion=?
                 WHERE id=?`,
              [pdfUrl, xmlUrl, ...datosCfdi, existente.id]);
      ...
      // ⚠️ Este catch envuelve TODO el bloque de arriba, no solo la espera del
      // correo: si el correo SÍ llegó y lo que reventó fue R2 o el INSERT, el
      // mensaje "no llegó correo" es mentira y manda a diagnosticar al lado
      // equivocado (se perdieron horas con el #380 por esto). Se registra el
      // error real: si NO es "Timeout"/"No se encontró correo", el correo llegó
      // y el fallo es nuestro — y el CFDI ya quedó marcado como leído.
      console.log(`⚠️ Job IMAP: no llegó correo para ticket #${ticket.id} — evaluando estrategias... (causa: ${imapErr.code || ''} ${imapErr.message})`);
