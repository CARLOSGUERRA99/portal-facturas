...
async function enviarCorreo(opts) {
  if (process.env.BREVO_API_KEY) {
    ...
    // Brevo acepta cc/bcc con la misma forma que `to`. Sin esto, un opts.cc se
    // ignoraba EN SILENCIO: nodemailer lo habría respetado, pero en producción
    // manda Brevo, así que la copia nunca salía y nadie se enteraba.
    for (const campo of ['cc', 'bcc']) {
      if (!opts[campo]) continue;
      const lista = (Array.isArray(opts[campo]) ? opts[campo] : String(opts[campo]).split(','))
        .map(e => String(e).trim()).filter(Boolean).map(email => ({ email }));
      if (lista.length) body[campo] = lista;
    }
