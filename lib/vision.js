...
const PROMPT_NETPAY_REFERENCIA = `Extrae estos datos de un ticket de gasolinera con terminal NetPay (Enerfuel Tech / Enerser y marcas que usan su plataforma). Responde SOLO JSON sin texto adicional:
{
  ...
  "fecha": "DD/MM/YYYY",
  "referencia": "⚠️ EL DATO CLAVE. Es el código alfanumérico que sigue a la palabra 'Referencia:' al FINAL del ticket, justo debajo de 'PAGINA(S) PARA FACTURAR' y de la URL. Son 15 o 16 caracteres SIN guiones (ej. '06922132266334FC', '049847152458CE1'). NO confundir con el 'Folio:' de la parte de arriba, que lleva guiones y pertenece a la terminal bancaria: ese NO sirve para facturar. ⚠️ CUENTA LOS CARACTERES antes de responder: si te salen menos de 15, RELÉELO — lo más fácil de perder es un dígito repetido, porque '888' y '88' se ven casi igual en papel térmico (pasó de verdad: se leyó '02647439888232' donde decía '026474398888232' y el portal contestó 'no se encontró el consumo' con la referencia buena). Si tras releer sigues sin poder contar con certeza, ponla en campos_dudosos en vez de entregar una versión corta.",
  ...
  carljr: `Extrae estos datos del ticket de Carl's Jr (ICR S.A. de C.V.). Responde SOLO JSON sin texto adicional:
{
  ...
  "portalUrl": "URL del portal de facturación si aparece (egridhub.com:6027/icr — el portal nuevo desde ago-2026 — o facturacion4.icr.mx), o null",
  "portal": "carljr",
${INSTRUCCION_CONFIANZA}`,
...
${INSTRUCCION_CONFIANZA}`,
  ...
  // Grupo CADISA / "AutoFacturas RADEC". El ticket trae TRES números parecidos
  // (Folio, FolioCV y Codigo) y el portal pide dos concretos; confundirlos hace
  // que no encuentre el consumo.
  cadisa: `Extrae estos datos del ticket de gasolinera RADEC / Grupo CADISA (dice "RADEC" en el encabezado y factura en un portal propio de la estación). Responde SOLO JSON sin texto adicional:
{
  "comercio": "la razón social de la estación, tal cual la línea de arriba",
  "estacion": "el número de la línea 'E.S.: ####' del encabezado, solo dígitos, o null",
  "fecha": "la de 'Fecha:' en DD/MM/YYYY",
  "folio": "⚠️ OBLIGATORIO: el número de la línea 'Folio:' (ej. '5032700'). NO el 'FolioCV:', que va justo debajo y se parece mucho.",
  "codigo": "⚠️ OBLIGATORIO: el número CORTO de la línea 'Codigo:' (3 o 4 dígitos, ej. '2259'). A veces se repite abajo como 'Codigo Web:'. NO confundirlo con el folio ni con el FolioCV.",
  "litros": número de la columna Volumen,
  "total": número de 'TOTAL', sin signos,
  "formaPago": "⚠️ lo que diga la línea de la VENTA, no el voucher del banco: si dice 'VENTA DE CONTADO' devuelve 'Efectivo'. El portal valida contra lo que registró la estación, y los tickets de contado se facturan como Efectivo aunque venga pegado un voucher de tarjeta.",
  "sitioFacturacion": "la URL que imprime el ticket para facturar (ej. 'www.rindemas.mx' o 'www.estacionpaloverde.com'), o null",
  "portal": "cadisa",
${INSTRUCCION_CONFIANZA}`,
  // Grupo Centra (Karmi) es el ÚNICO portal de la colección que pide la HORA
  // del ticket: sin ella su "Cargar" no encuentra el consumo. Por eso tiene
  // prompt propio en vez de caer en el genérico.
  grupocentra: `Extrae estos datos del ticket de gasolinera de Operadora Río Colorado / Grupo Centra (factura en facturacion.grupocentra.mx). Responde SOLO JSON sin texto adicional:
{
  "comercio": "la línea 'E.S. #### NOMBRE' completa del encabezado",
  "estacion": "⚠️ OBLIGATORIO: SOLO el número de la línea 'E.S. ####' del encabezado (ej. de 'E.S. 7870 SONOYTA' devuelve '7870'). Es como el portal identifica la sucursal.",
  "ciudad": "⚠️ SIEMPRE null. NO deducir la ciudad del domicilio impreso: ese domicilio es el del OPERADOR (todos los tickets dicen San Luis Río Colorado) y NO el de la estación — la estación 7870 del ticket que dice 'San Luis Río Colorado' está en realidad en SONOYTA. El bot recorre las 16 plazas del portal buscando el número de estación, que sí es fiable. Dar una ciudad equivocada aquí hace que el bot busque en la plaza que no es y falle.",
  "fecha": "⚠️ OBLIGATORIO: la fecha de la línea que va sola bajo el encabezado, en DD/MM/YYYY (esa línea trae fecha, hora y terminal juntas: '30/08/2026 20:31 PM, T3')",
  "hora": "⚠️ OBLIGATORIO Y CRÍTICO: la hora de esa MISMA línea, en formato HH:MM de 24 horas (de '20:31 PM' devuelve '20:31'; de '07:40 AM' devuelve '07:40'). El portal NO encuentra el consumo sin la hora exacta, así que si no se lee con claridad hay que decirlo en campos_dudosos.",
  "folio": "⚠️ OBLIGATORIO: el número de la línea 'Folio: #######' (el mismo que se repite entre paréntesis como '(Ticket: #######)')",
  "litros": número de la línea de producto,
  "total": número de 'Total:', sin signos,
  "formaPago": "lo que dice 'Tipo de Pago:' — 'Efectivo', 'Tarjeta Debito' o 'Tarjeta Credito'",
  "portalUrl": "https://facturacion.grupocentra.mx/Karmi_FacturacionWeb",
  "portal": "grupocentra",
${INSTRUCCION_CONFIANZA}`,
  // El portal pide TRES datos (estación, ticket y Web ID) y el prompt genérico
  // solo sacaba el ticket. El #357 se quedó parado por un dato de 4 dígitos que
  // estaba impreso en la foto desde el primer día.
  qualligas: `Extrae estos datos del ticket de gasolinera que factura en estacion.qualligas.com. Responde SOLO JSON sin texto adicional:
{
  "comercio": "la razón social de arriba del ticket (ej. 'GRUPO GASOLINERO DEL PACIFICO, S.A. DE C.V.'). ⚠️ QualliGas NO es el comercio: es el proveedor del software de facturación y aparece abajo como separador.",
  "estacion": "⚠️ OBLIGATORIO: el número de la línea 'ES:' (ej. '13920'). Sale otra vez al pie dentro de la URL 'estacion.qualligas.com/13920'. Es la ruta del portal: sin él no se sabe a qué estación ir.",
  "fecha": "la fecha que acompaña a 'No. TICKET', en DD/MM/YYYY",
  "folio": "⚠️ OBLIGATORIO: el número de 'No. TICKET:' (ej. '860670'), repetido al pie como 'TICKET:'. NO el 'FOLIO CARGA' de la zona de ventas, que es otro número parecido.",
  "webId": "⚠️ OBLIGATORIO Y ES EL QUE SIEMPRE SE OLVIDA: el número de la línea 'WEB ID:' del pie, justo debajo de 'TICKET:' (ej. '0927'). Suelen ser 4 dígitos y PUEDE LLEVAR CEROS A LA IZQUIERDA — devuélvelos tal cual. El portal exige Ticket Y Web ID: con uno solo no deja facturar. Si de verdad no lo ves, null, pero míralo dos veces antes.",
  "total": número de 'TOTAL:', sin signos,
  "portalUrl": "estacion.qualligas.com/ seguido del número de estación",
  "portal": "qualligas",
${INSTRUCCION_CONFIANZA}`,
  ...
  facturagas: `Extrae estos datos del ticket de gasolinera que factura en app.facturagas.net / ControlGAS (MIGASOLINA y similares). Responde SOLO JSON sin texto adicional:
{
  ...
  "permisoCRE": "⚠️ el número del permiso 'PERMISO C.R.E.: PL/####/EXP/ES/AAAA' (devuelve SOLO el número de en medio, ej. '5218'). Muchos tickets NO traen la línea con la clave de estación arriba; en esos el permiso es la única forma de identificarla, porque la clave que usa el portal es ese mismo número con prefijo (PL/5218 → estación 'P05218: SANCHEZ HERMANOS'). Si no aparece, null.",
  "sitioFacturacion": "la URL que el ticket imprime para facturar (ej. 'WWW.GASOLINERASMADRID.COM' o 'WWW.SVGASOLINERAS.COM'), o null. NO todas las estaciones ControlGAS están en app.facturagas.net; varias tienen su propio sitio y ahí hay que ir.",
  "fecha": "la de 'FECHA:' en DD/MM/YYYY",
  ...
  "portal": "facturagas",
${INSTRUCCION_CONFIANZA}`,
  // G500 corre ControlGAS igual que facturagas.net, PERO pide un folio distinto
  // y eso costó tres intentos fallidos: el portal NO encuentra el ticket con el
  // número de la línea "FOLIO :" (7 dígitos) — quiere el de 8 dígitos que va
  // ENTRE PARÉNTESIS después de la hora. Justo lo contrario de lo que dice el
  // prompt de facturagas, que para SU portal sí es correcto. Confirmado el
  // 08-sep-2026 con los tres tickets de Servicio Gastur (54871740, 54813560,
  // 54834890 funcionaron; 5407174, 5481356, 5403409 no).
  g500: `Extrae estos datos del ticket de gasolinera G500 Network (Servicio Gastur y otras estaciones G500; el ticket lleva el logo G500 y dice "FACTURA EN LINEA: www.g500network.com"). Responde SOLO JSON sin texto adicional:
{
  "comercio": "razón social de la estación, la línea bajo el logo G500",
  "fecha": "la de 'FECHA :' en DD/MM/YYYY",
  "folio": "⚠️ OBLIGATORIO Y OJO CON ESTE: NO es el número de la línea 'FOLIO :'. Es el número de 8 dígitos que aparece ENTRE PARÉNTESIS al final de la línea 'FECHA :', después de la hora (ej. 'FECHA : 07/09/2026, 11:26 (54871740)' → '54871740'). Con el de la línea 'FOLIO :' el portal responde 'No se encuentra el ticket ingresado'.",
  "folioImpreso": "el número de la línea 'FOLIO :', solo como respaldo. Puede ser null.",
  "webId": "⚠️ OBLIGATORIO: el número de la línea 'WEB ID :' (ej. '78183083'). El portal solo acepta 5 u 8 dígitos exactos, así que cuéntalos; si no puedes, ponlo en campos_dudosos.",
  "nota": "el número que sigue a 'NOTA #' (ej. '1316055')",
  "permisoCRE": "el valor de 'PERMISO C.R.E.:' (ej. 'PL/10942/EXP/ES/2015') — identifica la estación en el portal",
  "total": número de 'TOTAL:', sin signos,
  "portalUrl": "https://g500facturagas.azurewebsites.net",
  "portal": "g500",
${INSTRUCCION_CONFIANZA}`,
  // Gasolineros.mx (Grupo Timex) pide ESTACIÓN + "Número de secuencia". Las dos
  // se leían mal: la estación no se capturaba y como secuencia se mandaba el
  // "No. Ticket", que el portal rechaza con "Número de secuencia inválida".
  // La secuencia real es la línea "SECUENCIA FACTURACION" del pie, que es el
  // ticket MÁS un sufijo corto (1465883-17a, 7120186-6a0). En los tres tickets
  // de sep-2026 hubo que leerla a mano de la foto, y en uno el sufijo salió con
  // B donde había un 8 — de ahí el aviso de no confundir letras y dígitos.
  gasolineros: `Extrae estos datos del ticket de gasolinera que factura en gasolineros.mx / Grupo Timex (La Cuesta-Grupo Hispánica, MABA San Francisco y similares; el ticket puede citar hercorgas.com o grupomaba.com). Responde SOLO JSON sin texto adicional:
{
  "comercio": "razón social de la estación, las primeras líneas del ticket",
  "estacion": "⚠️ OBLIGATORIO: SOLO los dígitos que siguen a 'ESTACION:' o 'ESTAC. DE SERV. NO.' en el encabezado (ej. 'ESTACION: 13236' → '13236'). Sin este número el portal no deja ni empezar.",
  "secuencia": "⚠️ OBLIGATORIO Y EL DATO MÁS IMPORTANTE: la línea 'SECUENCIA FACTURACION' o 'Secuencia:' del PIE del ticket, COMPLETA y tal cual, con su guion y su sufijo (ej. '1465883-17a', '7120186-6a0', '7128015-68c'). NO es el 'No. Ticket' de arriba: ese es solo la primera mitad y el portal lo rechaza. El sufijo mezcla dígitos y letras minúsculas: NO conviertas 8 en B, 0 en O, 1 en l ni al revés — si un carácter no se distingue con certeza, ponlo en campos_dudosos en vez de adivinarlo.",
  "folio": "el número que sigue a 'No. Ticket:' o 'Ticket:' (la primera mitad de la secuencia, sin sufijo)",
  "fecha": "DD/MM/YYYY",
  "total": número de 'Total:' o 'Importe:', sin signos,
  "portalUrl": "https://www.gasolineros.mx/facturacion/facturacion.aspx",
  "portal": "gasolineros",
${INSTRUCCION_CONFIANZA}`,
  ...
  orler: `Extrae estos datos del ticket de caseta de peaje de Sinaloa (Orler). Responde SOLO JSON sin texto adicional:
{
  ...
  "portal": "orler",
${INSTRUCCION_CONFIANZA}`,
  // Casetas del Fideicomiso Tramo Carretero (Centinela-Rumorosa, BC). El ticket
  // NO imprime la URL de su portal, por eso salia como "desconocido" y el
  // agente dedujo un dominio (ficacentinela.com.mx) que no existe.
  fiarum: `Extrae estos datos del ticket de caseta del Fideicomiso Tramo Carretero Centinela-Rumorosa (FIARUM, Baja California). Responde SOLO JSON sin texto adicional:
{
  "comercio": "Fideicomiso Tramo Carretero y el nombre de la plaza, ej. 'Fideicomiso Tramo Carretero - Plaza Centinela-Rumorosa'",
  "autopista": "nombre del tramo, ej. 'Centinela-Rumorosa', o null",
  "fecha": "la del ticket, en DD/MM/YYYY",
  "hora": "HH:MM:SS si aparece, o null",
  "folio": "el numero de FOLIO, con sus ceros a la izquierda si los lleva. Leelo digito por digito.",
  "carril": "⚠️ OBLIGATORIO, el portal no busca sin el: el numero de CARRIL. Solo los digitos.",
  "total": numero del importe, sin signos,
  "portalUrl": "https://fiarumfacturas.com.mx/cfdi/autofacturas",
  "portal": "fiarum",
${INSTRUCCION_CONFIANZA}`,
  // Caseta de San Luis Rio Colorado. Portal nuevo (qrplus) desde sep-2026: por
  // el camino sin WebID pide caseta, carril y hora ademas del folio.
  puentecolorado: `Extrae estos datos del ticket de la caseta de peaje de San Luis Rio Colorado (Fideicomiso Puente Colorado, Gobierno de Sonora). Responde SOLO JSON sin texto adicional:
{
  "comercio": "Puente Colorado / caseta San Luis Rio Colorado",
  "fecha": "la del ticket, en DD/MM/YYYY",
  "hora": "⚠️ HH:MM:SS — el portal la pide para localizar el ticket. Si de verdad no aparece, null.",
  "folio": "el numero de FOLIO, con sus ceros a la izquierda. Leelo digito por digito.",
  "caseta": "nombre o numero de la caseta/plaza, ej. 'SAN LUIS RIO COLORADO', o null",
  "carril": "⚠️ el numero de CARRIL — el portal lo exige cuando el ticket no trae WebID. Solo los digitos, o null si no aparece.",
  "webId": "si el ticket imprime un WEB ID (codigo en 5 grupos separados por guiones), copialo TAL CUAL con los guiones: es el atajo del portal y se salta los demas campos. Si no aparece, null.",
  "total": numero del importe, sin signos,
  "portalUrl": "https://www.qrplus.com.mx/SONORA_Facturacion/FormsFacturacion/FWizExprFacturacion.aspx",
  "portal": "puentecolorado",
${INSTRUCCION_CONFIANZA}`,
  // El ticket imprime laparisina.com.mx (sitio corporativo, no factura): la URL
  // buena es el subdominio, por eso va fija aqui y no se lee de la foto.
  laparisina: `Extrae estos datos del ticket de Parisina (Grupo Parisina S.A. de C.V., telas y merceria). Responde SOLO JSON sin texto adicional:
{
  "comercio": "Parisina y la sucursal si aparece",
  "fecha": "DD/MM/YYYY",
  "folio": "el numero de folio/ticket de la compra",
  "total": numero sin signos,
  "portalUrl": "https://facturacion.laparisina.com.mx/",
  "portal": "laparisina",
${INSTRUCCION_CONFIANZA}`,
  // Autopista Tijuana-Tecate (Grupo IDEAL). NO es PINFRA aunque sea caseta: el
  // ticket #393 se perdio semanas por esa confusion.
  tijuanatecate: `Extrae estos datos del ticket de la autopista Tijuana-Tecate (Grupo IDEAL). Responde SOLO JSON sin texto adicional:
{
  "comercio": "el encabezado del ticket, p.ej. 'TIJUANA - TECATE'",
  "autopista": "la del encabezado, TAL CUAL: TIJUANA - TECATE, PASO DEL AGUILA, ESPERANZAS o PASO DEL AGUILA BIS. El portal la llama \"Entronque\".",
  "fecha": "DD/MM/YYYY",
  "hora": "HH:MM:SS — va junto a la fecha, en la misma linea",
  "folio": "el de la linea Folio:, CON sus ceros a la izquierda (p.ej. 0000360201). ⚠️ NO el de No:, que es otro numero parecido justo antes en la MISMA linea.",
  "carril": "el codigo de la linea Carril:, con su letra (p.ej. 1009A). Es obligatorio: el portal lo pide como desplegable.",
  "total": numero de Total MN, sin signos ni ceros a la izquierda,
  "portalUrl": "https://www.facturaciontij-tkt.com.mx/facturacion",
  "portal": "tijuanatecate",
${INSTRUCCION_CONFIANZA}`,
  desconocido: `Extrae los datos que puedas de este ticket. Si reconoces el portal, identifícalo.
Portales conocidos: oxxo (tiendas OXXO), arco (gasolineras ARCO, portal buzonfacturas.com), gasmaz (gasolineras Gasmaz/RedMax/NexusFuel), farmaciaguadalajara (Farmacias Guadalajara), benavides (Farmacias Benavides), homedepot (Home Depot México), rendichicas (gasolineras con QR a rendilitros.com o rendichicas.com), panama (Panamá Restaurante y Pastelería, portal grupopanama.mx), carljr (Carl's Jr / ICR S.A. de C.V., portal egridhub.com:6027/icr), autozone (AutoZone de México, portal autozone.cdc.origon.cloud), 7eleven (tiendas 7-Eleven, "7 Eleven Mexico SA de CV", portal e7-eleven.com.mx — el folio es un código de 35 dígitos), caffenio (cafeterías CAFFENIO drive, portal facturaciondrive.caffenio.com — requiere folio + código de facturación + nombre del drive).
Responde SOLO JSON sin texto adicional:
{
  ...
  "portal": "oxxo|oxxogas|arco|gasmaz|farmaciaguadalajara|benavides|homedepot|rendichicas|panama|carljr|autozone|7eleven|igasfac|caffenio|sushito|fiarum|puentecolorado|tijuanatecate|desconocido",
${INSTRUCCION_CONFIANZA}`,
};
...
const camposPorPortal = {
  ...
  // El Web ID es un campo aparte del portal: con solo el ticket no busca.
  qualligas:           ['estacion', 'folio', 'webId', 'total'],
  facturagas:          ['estacionNombre', 'folio', 'webId', 'total'],
  gashr:               ['folio', 'referencia', 'total'],
  // Sin la estación el portal no arranca, y sin la SECUENCIA completa (con su
  // sufijo) rechaza el consumo con "Número de secuencia inválida".
  gasolineros:         ['estacion', 'secuencia', 'total'],
  // El folio de G500 es el de 8 digitos entre parentesis, no el de la linea FOLIO.
  g500:                ['folio', 'webId', 'total'],
  ...
  grupocentra:         ['estacion', 'fecha', 'hora', 'folio', 'total'],
  cadisa:              ['folio', 'codigo', 'total'],
  // Sin carril el portal de FIARUM no encuentra el cruce, igual que Orler.
  fiarum:              ['folio', 'carril', 'fecha', 'total'],
  // Camino sin WebID: el portal exige tambien caseta, carril y hora.
  puentecolorado:      ['folio', 'carril', 'fecha', 'hora', 'total'],
  laparisina:          ['folio', 'fecha', 'total'],
  // El portal pide autopista, carril, folio, fecha-hora y monto.
  tijuanatecate:       ['folio', 'carril', 'fecha', 'hora', 'total'],
...
function repararReferenciaNetPay(datosOCR) {
  ...
  if (ref.length === 5 + noTicket.length + 4) {
    ...
    // ⚠️ NO se quita el "dudoso": esta reparación solo verifica 6 de los 15
    // caracteres (el nº de ticket del centro). La estación y sobre todo el
    // VERIFICADOR de 4 caracteres del final siguen siendo la lectura cruda del
    // modelo, y son justo donde se cuelan las confusiones letra↔dígito.
    //
    // Medido en el ticket #387: impreso "052007273914EAD", el OCR entregó
    // "052007279314E40" (dos fallos: 93↔39 en el centro y "AD"→"40" en el
    // verificador). Esta función arregló el centro, borró la marca de duda y el
    // bot salió a buscar con un verificador equivocado; el portal contestó "No
    // se encontró el consumo" y el ticket se archivó como vencido cuando seguía
    // perfectamente facturable (se comprobó en vivo dos días después).
    //
    // Si hubo que reparar, la lectura de ESTE campo ya se demostró mala: dejarla
    // marcada hace que el modal de confirmación (public/dashboard.html) resalte
    // la referencia, que es exactamente el campo que hay que mirar en la foto.
...
async function procesarImagenTicket(imageBuffer, mimeType) {
  ...
  try {
    ...
    if (portalDetectado === "desconocido" && urlQR) {
      ...
      else if ((urlLow.includes("egridhub") && urlLow.includes("/icr")) || urlLow.includes("facturacion4.icr.mx") || urlLow.includes("icr.mx")) portalDetectado = "carljr";
      // gasolineros.mx es la plataforma; el ticket impreso suele citar el sitio
      // de la CADENA (hercorgas.com, grupomaba.com), no el del portal.
      else if (urlLow.includes("g500network") || urlLow.includes("g500facturagas") || urlLow.includes("servgastur")) portalDetectado = "g500";
      else if (urlLow.includes("gasolineros.mx") || urlLow.includes("hercorgas.com") || urlLow.includes("grupomaba.com")) portalDetectado = "gasolineros";
      ...
      else if (urlLow.includes("enerser.com.mx")) portalDetectado = "enerser";
      else if (urlLow.includes("grupocentra")) portalDetectado = "grupocentra";
      else if (urlLow.includes("rindemas") || urlLow.includes("estacionpaloverde") || urlLow.includes("palov966facturas")) portalDetectado = "cadisa";
      ...
      else if (urlLow.includes("fiarumfacturas")) portalDetectado = "fiarum";
      else if (urlLow.includes("qrplus.com.mx") || urlLow.includes("puentecolorado")) portalDetectado = "puentecolorado";
      else if (urlLow.includes("facturaciontij-tkt") || urlLow.includes("tijuanatecate.com.mx")) portalDetectado = "tijuanatecate";
