...
// Busca un ticket duplicado (mismo usuario, comercio y folio, no en error).
// Devuelve el ticket existente o null. excludeTicketId: ignora el propio ticket.
async function buscarDuplicado(userId, datosOCR, excludeTicketId = null) {
  ...
  if (!folioUnico || !datosOCR.comercio) return null;
  try {
    const [dups] = await db.query(
      ...
      [userId, datosOCR.comercio, excludeTicketId || 0, String(folioUnico)]
    );
