...
async function procesarImagenTicket(imageBuffer, mimeType) {
  ...
  const idFacturacion = datosOCR.folio || datosOCR.codigoTicket || datosOCR.referencia ||
                        datosOCR.idFacturacion || datosOCR.folioFactura || datosOCR.idVenta;
  const sinIdentificador = !String(idFacturacion || '').trim();
  if (sinIdentificador) console.log('🔎 Sin folio/referencia que mandar al portal — va a confirmación');

  const requiereConfirmacion =
    ticketsEnFoto > 1 ||     // se perdería el resto de compras de la foto
    sinIdentificador         // no hay nada que enviar
      ? 1 : 0;
