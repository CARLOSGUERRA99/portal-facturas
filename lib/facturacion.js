...
async function ejecutarFacturacion(ticketId, userId) {
  try {
    ...
    if (resultado.ok && resultado.procesandoCorreo) {
      // Se limpia el error anterior: si no, un ticket que acaba bien conserva
      // en pantalla el motivo del intento fallido y parece roto cuando no lo
      // está (los tres de Orler quedaron facturados mostrando un 429 viejo).
      await db.query("UPDATE tickets SET status = 'procesando_correo', procesando_correo_desde = NOW(), error_msg = NULL, reintento_programado = NULL WHERE id = ?", [ticketId]);
