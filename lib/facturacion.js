...
// Saber si un CAPTCHA tiene solver decide si el ticket se reintenta o va a manual.
const { esResoluble } = require('./captcha');
// ⚠️ Carga PEREZOSA de las colas: queues/index.js revienta al importarse si no
// hay REDIS_URL, y el Redis de Railway solo existe dentro de su red privada.
// Con el require arriba, este módulo era imposible de ejecutar desde una
// máquina local — justo lo que el CLAUDE.md manda hacer para depurar sin
// desplegar a ciegas. Se usa en un único sitio, así que se pide ahí.
// worker.js ya hace lo mismo en rescatarTicketsSinEncolar().
const encolarAgente = (...args) => require("../queues").encolarAgente(...args);
...
function emailFacturaLista({ nombre, comercio, xmlUrl, pdfUrl, clienteNombre }) {
  // El sistema es multi-cliente (GPN, DGA, ...); el correo debe llevar la
  // marca del cliente dueño del ticket, no siempre GPN. Fallback a GPN solo
  // para tickets sin cliente asignado (users.cliente_id NULL).
  const marca = clienteNombre || 'GPN Pinturas y Recubrimientos';
  return `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
    <div style="background:#3B6D11;padding:20px;border-radius:12px 12px 0 0;">
      <h2 style="color:#fff;margin:0;">${marca}</h2>
...
async function ejecutarFacturacion(ticketId, userId) {
  try {
    ...
    // ⚠️ "OXXO GAS" CONTIENE "OXXO", Y NO ES OXXO.
    //
    // Esta validación previa es la de la TIENDA de conveniencia: exige un
    // idVenta de 11 caracteres que solo imprimen sus tickets. El filtro
    // `comercio.includes('oxxo')` la disparaba también para las gasolineras, que
    // facturan en otro portal (facturacion.oxxogas.com) y ni siquiera tienen ese
    // campo. Resultado: el ticket moría aquí con "idVenta inválido (0 chars):
    // undefined" y NUNCA llegaba a bots/oxxogas.js — el bot correcto no se
    // ejecutaba jamás y el mensaje culpaba al OCR. Le pasó al #344.
    //
    // El router de bots/index.js ya tiene esta misma precaución (comprueba OXXO
    // GAS antes que OXXO); faltaba aquí.
    const comercioLower = (ticket.comercio || '').toLowerCase();
    const esOxxoGas = datos.portal === 'oxxogas' ||
      comercioLower.includes('oxxo gas') || comercioLower.includes('oxxogas') ||
      String(datos.portalUrl || ticket.portal_url || '').toLowerCase().includes('oxxogas');

    if (!esOxxoGas && (datos.portal === 'oxxo' || comercioLower.includes('oxxo'))) {
    ...
    if (resultado.ok && resultado.procesandoCorreo) {
      // ⚠️ EL DATO QUE NO SE PUEDE PERDER. Varios portales devuelven, junto al
      // "esperando correo", el identificador con el que luego se recupera el
      // CFDI si el correo nunca llega: el folio interno BXI-xxxxxxx de
      // BuzonFacturas/ARCO, o el UUID de SoftRestaurant. Antes se descartaba, y
      // sin él la factura es IRRECUPERABLE: buzonfacturas.com exige RFC **y**
      // folio interno para consultar, y volver a generarla emitiría un
      // duplicado. Le pasó al ticket #373. Ahora se conserva en error_msg, que
      // es el único campo de texto libre que sobrevive al ciclo.
      const rastro = [resultado.folioGenerado && `folio interno ${resultado.folioGenerado}`,
                      resultado.uuid && `UUID ${resultado.uuid}`].filter(Boolean).join(' · ');
      const nota = rastro
        ? `FACTURA GENERADA (${rastro}) — esperando el correo. NO RELANZAR: si el correo no llega, recupérala con ese identificador; reintentar emitiría un CFDI duplicado.`
        : null;
      await db.query(
        "UPDATE tickets SET status = 'procesando_correo', procesando_correo_desde = NOW(), error_msg = ?, reintento_programado = NULL WHERE id = ?",
        [nota, ticketId]
      );
      await registrarIntento(ticketId, botNombre, 'procesando_correo',
        rastro ? `Factura generada (${rastro}) — esperando correo` : 'Factura generada — esperando correo', duracionMs);
    ...
    if (resultado.ok) {
      ...
      await db.query(
        `INSERT INTO facturas (user_id, ticket_id, comercio, pdf_url, xml_url, status,
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        // ⚠️ `facturas.comercio` es varchar(50) y hay comercios más largos: el
        // #365 ("Comedor Valencia (Mariel Alexandra Valencia Eraldes)") mide 52.
        // Sin este slice, MySQL rechaza el INSERT con "Data too long", la
        // excepción sube, y una factura YA TIMBRADA se registra como error
        // genérico —> reintento a medianoche —> segundo CFDI. Dos caracteres de
        // más bastaban para duplicar una factura.
        [userId, ticketId, String(ticket.comercio || '').slice(0, 50), pdfUrl, xmlUrl, 'completado',
      ...
      await db.query("UPDATE tickets SET status = 'procesado', error_msg = NULL, reintento_programado = NULL WHERE id = ?", [ticketId]);
      try {
        if (ticket.email) {
          const marca = ticket.cliente_nombre || 'GPN Pinturas y Recubrimientos';
          await enviarCorreo({
            from: `"${marca}" <buzonfacturas@serviciosga.site>`,
            to: ticket.email,
            subject: `✅ Tu factura está lista — ${marca}`,
            html: emailFacturaLista({ nombre: ticket.user_nombre, comercio: ticket.comercio, xmlUrl, pdfUrl, clienteNombre: marca }),
          });
        }
      } catch {}
    ...
    if (resultado.error_code === 'ticket_vencido') {
      // Los bots sacan este correo del texto del portal, y ahí suele venir
      // pegado a puntuación ("...@compuexpertos.net;" en Pollo Feliz). Tal
      // cual no pasa la validación de la ventana "Solicitar por correo" y el
      // ticket se queda sin poder pedirse. Se limpia una vez aquí, para todos
      // los bots, en vez de parchear cada extracción por separado.
      const emailContacto = String(resultado.email_contacto || '')
        .trim().replace(/^[<"'\s]+|[>"'\s.,;:]+$/g, '') || null;
      await db.query(
    ...
    // ── CAPTCHA ────────────────────────────────────────────────────────────
    // ⚠️ Antes esta rama mandaba a manual CUALQUIER error_code 'captcha' y
    // ponía reintento_programado = NULL, o sea: nunca se reintentaba. Se
    // escribió cuando el repo no resolvía captchas. Derogado el 13-sep-2026:
    // hoy resolvemos reCAPTCHA v2, Turnstile e imagen, así que un fallo de
    // captcha en esos portales es un fallo TRANSITORIO (el solver falla a
    // veces) y merece el mismo reintento a medianoche que cualquier otro
    // error. Seguir mandándolos a manual era tirar tickets que sí salen al
    // segundo intento.
    //
    // Solo se apaga el reintento cuando el tipo de captcha NO tiene solver
    // (lib/captcha.js → SOLVERS): ahí reintentar sí es quemar dinero sin
    // posibilidad de acertar. Es el caso medido del texto sensible a
    // mayúsculas de Parisina/QualliGas.
    if (resultado.error_code === 'captcha') {
      const tipo = resultado.captcha_tipo || null;
      const puedeReintentarse = !tipo || esResoluble(tipo);
      const medianocheCaptcha = puedeReintentarse ? proximaMedianoche() : null;

      await db.query(
        "UPDATE tickets SET status = 'error', error_msg = ?, reintento_programado = ? WHERE id = ?",
        [(resultado.msg || 'Falló el CAPTCHA').slice(0, 500), medianocheCaptcha, ticketId]
      );
      await registrarIntento(ticketId, botNombre, 'error', `captcha|${tipo || '?'}|${resultado.portal_url || ''}`, duracionMs);
      await crearNotificacion(userId, 'factura_error', puedeReintentarse
        ? `No se pudo pasar el CAPTCHA de ${ticket.comercio || 'este comercio'} esta vez. Reintentaremos esta noche a las 12:00 am.`
        : `Tu ticket de ${ticket.comercio || 'este comercio'} debe facturarse MANUALMENTE: su CAPTCHA todavía no se puede resolver. Tus datos ya están extraídos y listos.`
      ).catch(() => {});
    ...
    // ── EL CFDI YA EXISTE, SOLO FALTA EL ARCHIVO ─────────────────────────────
    // Es el caso más peligroso del sistema y no tenía rama propia: el bot
    // timbró de verdad (tiene el folio, a veces el UUID) pero no consiguió
    // bajar el XML/PDF. Los bots lo devolvían como `reintentar_despues`, que es
    // precisamente el código que reintenta CADA NOCHE PARA SIEMPRE: el ticket
    // volvía al portal noche tras noche emitiendo un CFDI nuevo cada vez. El
    // mensaje decía "NO reintentar", pero eso lo lee un humano, no el sistema;
    // el sistema solo mira el error_code. Pasaba en bots/topgas.js:274 y :289.
    //
    // Aquí NO se reintenta nunca: el ticket queda a la vista de una persona con
    // el identificador necesario para bajar la factura a mano y asociarla con
    // scripts/asociar-cfdi.js.
    if (resultado.error_code === 'timbrado_sin_archivos') {
      await db.query(
        `UPDATE tickets SET status = 'error', error_msg = ?, reintento_programado = NULL,
                            requiere_confirmacion = 0 WHERE id = ?`,
        [(resultado.msg || 'CFDI timbrado pero no se pudo descargar').slice(0, 500), ticketId]
      );
      await registrarIntento(ticketId, botNombre, 'error', `timbrado_sin_archivos|${resultado.msg || ''}`, duracionMs);
      await crearNotificacion(userId, 'factura_error',
        `Tu factura de ${ticket.comercio || 'este comercio'} SÍ se generó, pero no pudimos descargar el archivo. La recuperaremos manualmente — no hace falta que subas el ticket otra vez.`
      ).catch(() => {});
      return { ok: false, error_code: 'timbrado_sin_archivos', msg: resultado.msg };
    }
...
function resumenAgente(resultado, comercioNombre) {
  // ⚠️ Aquí el agente SOLO llega con resultado.captcha cuando el tipo no tiene
  // solver (ver agentes/analizador.js). Antes llegaba con cualquier captcha y
  // el mensaje decía "no se puede automatizar": derogado el 13-sep-2026, hoy
  // reCAPTCHA v2, Turnstile e imagen se resuelven y el alta sigue adelante.
  // El motivo tiene que seguir conteniendo la palabra "captcha": es lo que hace
  // que Validación Manual lo clasifique como "facturar a mano".
  if (!resultado.ok && resultado.captcha) {
    return {
      corto: `captcha: ${resultado.msg}`,
      usuario: `Tu ticket de ${comercioNombre} lo estamos facturando a mano — el portal pide una verificación para la que todavía no tenemos solver. Te avisamos en cuanto esté.`,
      admin: `🛑 ${comercioNombre} usa ${resultado.captcha_tipo || 'un CAPTCHA'}, para el que AÚN NO hay solver escrito (los que sí tienen están en lib/captcha.js → SOLVERS). No se gastó el alta. Factúralo desde Validación Manual, o escribe el solver y reintenta.`,
    };
  }
  if (!resultado.ok) {
