...
const PORTALES_FACTURABLES = [
  ...
  'allegro', 'allegrecaffe', 'allegrezonadorada', 'autozone', 'albatros', 'pinfra', '7eleven', 'dana', 'tufesa', 'bodegaaurrera',
  ...
  // ENERSER: bot escrito el 08-sep-2026 (bots/enerser.js) tras timbrar en vivo
  // los tickets #335 y #338. Antes tenía prompt de OCR pero ni bot ni routing.
  'enerser',
  // GRUPO CENTRA (Karmi): estaciones de Operadora Río Colorado en Sonora/BC.
  // Bot escrito el 08-sep-2026 (bots/grupocentra.js) tras timbrar los tickets
  // #320 (E.S. 7870 Sonoyta) y #343 (E.S. 8394 Obregón y Cuauhtémoc).
  'grupocentra',
  // TOPGAS (Kernotek): bot escrito el 08-sep-2026 tras timbrar el #339.
  'topgas',
  'qualligas',
  'estrellablanca',
  'lossenderos',
  ...
  'facturat',
  'redco',
  // GRUPO CADISA / AutoFacturas RADEC: una instancia por gasolinera en su
  // propio DDNS. Bot escrito el 08-sep-2026 tras timbrar #323 y #340.
  'cadisa', 'radec',
  // ORSAN (estaciones Mobil): exige cuenta, credenciales en .env. 09-sep-2026.
  'orsan',
  ...
  'capufe', 'igasfac',
  // LITTLE CAESARS: el reCAPTCHA v2 se resuelve con CapSolver desde el
  // 15/08/2026 (bots/littlecaesars.js). Antes pasaba el gate solo por la URL
  // (analytix360) cuando el OCR dejaba portal='desconocido'; con portal
  // bien detectado ('littlecaesars') quedaba FUERA y el ticket se atoraba.
  'littlecaesars',
  // OXXO GAS: entra al gate, pero HOY su bot no es autónomo — depende de que
  // OXXOGAS_CI_SESSION traiga una cookie de sesión abierta a mano. Sin esa
  // cookie corta ANTES de abrir Browserless y devuelve error_code 'captcha',
  // que en lib/facturacion.js apaga los reintentos y avisa "factúralo
  // manualmente". O sea: no quema saldo ni se cicla, pero mientras no haya
  // cookie fresca TODO ticket de OXXO GAS termina en error.
  // ⚠️ Ojo: esto es el estado actual, no un límite. El login lleva reCAPTCHA v2
  // y este repo SÍ lo resuelve desde el 15-ago (igual que Little Caesars, tres
  // líneas más arriba). Automatizarlo está aprobado — plan en CLAUDE.md,
  // "Pendientes", punto 0.
  'oxxogas',
  // FIARUM — caseta Centinela-Rumorosa (BC). El prompt propio de lib/vision.js
  // ya devuelve portal:'fiarum', asi que la rama de URLS_FACTURABLES (que solo
  // corre con portal 'desconocido') nunca se consultaria: sin esta linea el
  // ticket se queda en pendiente_confirmacion para siempre.
  'fiarum',
  'puentecolorado',
  'tijuanatecate',
];
const URLS_FACTURABLES = [
  'autozone', 'origon.cloud', 'albatrosautobuses', 'grupoalbatros', 'pinfrafacturacion', 'mefacturo.mx', 'elcaporal', 'allegre', 'sushio', 'analytix360',
  'tufesa', 'e7-eleven', 'softrestaurant.com', 'mefacturo.com',
  ...
  'facturagas.net', 'erfc.com.mx', 'enerfueltech.com', 'enerser.com.mx', 'grupocentra.mx', 'topgasmexico', 'topgas.kernotek', 'qualligas.com', 'estrellablanca.com.mx', 'lossenderos.com.mx', 'facturafranquicias', 'grupoarlosa', 'grupoa-losa', 'youbuy.mx', 'autofacturat.com.mx', 'gruporedco', 'grupohorizon', 'facturaenlinea.aspx', 'rindemas', 'estacionpaloverde', 'palov966facturas', 'orsan.com.mx', 'corporativoramcal', 'ramcal.no-ip',
  'facturaciondrive.caffenio', 'facturacioncapufe', 'capufe.gob.mx',
  '1gasfac', 'lgasfac', 'igasfac',
  // Red de seguridad por si el OCR cae al prompt generico y deja
  // portal='desconocido' pero si copia la URL.
  'fiarumfacturas',
  'qrplus.com.mx', 'puentecolorado',
  'facturaciontij-tkt',
];
